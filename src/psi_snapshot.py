...
def parse_response(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if inp_ms is None:
        ...
        if inp_ms is None:
            # fallback to TBT proxy is not ideal; better leave None
            pass
    if cls is None:
...
async def gather_psi(urls: List[str], api_key: str, strategies: List[str], concurrency: int, category: str, locale: str) -> List[Dict[str, Any]]:
...
def main():
    ...
    results = asyncio.run(gather_psi(urls, key, args.strategies, args.concurrency, args.category, args.locale))
